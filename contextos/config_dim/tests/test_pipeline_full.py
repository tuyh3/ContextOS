...
- build_config_dimension 把三 Phase 串成一条链, 离线只验主链(真跑覆盖见 C6 integration):
  ...
  * Phase B: 对注入的 oracle_tables 清单跑四路识别(path A 表名启发 / path B DDL 表注释,
    读随表清单注入的 t["comment"], D.5 去 live SQL 化 / path C+D RAG sparse 走 rag_search)->
    fuse_config_table 融合 -> 命中 high 的写 config_sources(source_type='db_table')。
...
- 评分标准(assert):
  ...
  2. stats['config_tables'] >= 1(SYS_CONFIG 被四路融合判 high)。
     ...
     path B 命中(注入 comment '系统配置表' 含 zh 关键词 '配置')+ path C 命中
...
- 自动脚本测试逻辑: sqlite in-memory engine + metadata.create_all; 注入 oracle_tables 静态
  清单(含 comment 字段, D.5 路 B 数据源)+ fake_search(模拟 03b sparse scoped_hits)。
  build 后 select config_sources 断言。engine_05 不给 -> 不回写。
...
def test_full_build_offline_with_fakes(tmp_path):
    ...
    # Phase B fakes: oracle 表清单(D.5: 路 B 注释随表清单注入 comment)+ search(RAG)
    oracle_tables = [{
        "owner": "UPC", "table": "SYS_CONFIG",
        "columns": ["EFFECTIVE_DATE", "STATUS"], "row_count": 10,
        "comment": "系统配置表",   # 路 B: 含 zh 信号词 '配置' -> 命中(源自 store table_metadata.comment)
    }]
    ...
    stats = build_config_dimension(
        ...
        oracle_tables=oracle_tables, rag_search=fake_search,
...
def test_full_build_populates_all_tables(tmp_path):
    ...
    - SYS_CONFIG fake: path A 命中(name_patterns CONFIG + 规则列 EFFECTIVE_DATE+STATUS >=2)+ path B
      注入 comment '配置表' 含 zh 信号词(D.5 store 注释)+ path C business_docs 命中行含 '配置' ->
      fuse >=0.6 且 >=2 路 -> high -> 写 db_table 源 + 触发 W5 证据 + W7 快照。
    - 评分标准(assert): 4 张表均非空。
      ...
      * config_items value_type='row'(W7): high 表 + row_count<=阈值 -> SELECT * -> snapshot_small
        每行一条 value_type='row'。fake_exec(D.5 后只服务 W7 SELECT *)返一行 {"K":"A","V":"1"}。
    ...
    def fake_exec(db, sql, **kw):
        # D.5 后 execute_query 只剩 W7 行快照(SELECT *); 路 B 注释改走注入的 comment
        return [{"K": "A", "V": "1"}]
    ...
    oracle_tables = [{"owner": "UPC", "table": "SYS_CONFIG",
                      ...
                      "comment": "配置表"}]   # 路 B: 含 zh 信号词 '配置'(D.5 store 注释)
