...
def _safe_sqlparse_format(statement, **kwargs):
    """[ContextOS patch 1] spec 附录 E.3.1: sqlparse>=0.5 对超大 SQL 抛
    "Maximum number of tokens exceeded (10000)"(reindent/strip_comments 排版
    美化步, 非解析必需)。pak 级 82 语句大 mapper 实测会触发; 失败时返回
    未美化原文, 不阻断展开。"""
    try:
...
def get_statement(mybatis_mapper, result_type='raw', **kwargs):
    ...
    if result_type == 'list':
        ...
        return _safe_sqlparse_format(statement, **kwargs)  # [ContextOS patch 1]
    else:
